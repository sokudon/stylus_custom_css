話題の生成ai　chatgpt/gemini/claude/lmsys(ばとるありーな)
chrome/edge /firefox用のあどおん、カスタムCSSです,仕様変更で使えなくなる可能性があるので(
https://chromewebstore.google.com/detail/stylus/clngdbkpkpeebahjckkjfobafhncgmne?hl=JA
https://addons.mozilla.org/ja/firefox/addon/styl-us/

ウェブからインストール
https://userstyles.world/user/strmdn
https://github.com/sokudon/stylus_custom_css

base64で好きな画像にry（）
https://ss1.xrea.com/sokudon.s17.xrea.com/
http://unzu127xp.web.fc2.com/base64.html

2025/03/06
chatgpt/gemini/claude/grok/xcom/lmsys/perplesive/copilot
ブックマークレット拡張についか,geminiとclaudeはドメイン制限が厳しいのでファイルのみ
ブックマークレットはtxtファイルです

user.css　chome/firefox/edget用あどおん　stylusようです
***.js  tampermonkey用です
***.txt ブックマークレット形式です,ブックマークにいれるやつ　モバイルのedgeだと動かないです

2025/02/20
ブックマークレット拡張がないスマフォぶらうざで動くかもしれない
2025/02/XX
gemini/claude tampermokey版を追加